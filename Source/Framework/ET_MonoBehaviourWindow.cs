...
namespace EVATransfer.Framework
{
    ...
    /// </summary>
    public abstract class ET_MBW : ET_MBE
    {
        #region "Constructors"
        internal ET_MBW()
            : base()
		...
		internal void DrawGUI()
		{
			...
			GUI.skin = ET_SkinsLibrary.CurrentSkin;
        ...
        private void DrawToolTip()
        {
            ...
            if (TooltipsEnabled && Visible && (strToolTipText != "") && ((TooltipDisplayForSecs == 0) || (fltTooltipTime < (Single)TooltipDisplayForSecs)))
            {
                ...
                GUIStyle styleTooltip = ET_SkinsLibrary.CurrentTooltip;
                ...
                if (TooltipMaxWidth > 0)
                {
                    ...
                    ET_SkinsLibrary.CurrentTooltip.CalcMinMaxWidth(contTooltip, out minwidth, out maxwidth); // figure out how wide one line would be
                    _TooltipPosition.width = Math.Min(TooltipMaxWidth - ET_SkinsLibrary.CurrentTooltip.padding.horizontal, maxwidth); //then work out the height with a max width
                    _TooltipPosition.height = ET_SkinsLibrary.CurrentTooltip.CalcHeight(contTooltip, TooltipPosition.width); // heres the result
                }
                else
                {
                    //calc the new width and height
                    Vector2 Size = ET_SkinsLibrary.CurrentTooltip.CalcSize(contTooltip);
