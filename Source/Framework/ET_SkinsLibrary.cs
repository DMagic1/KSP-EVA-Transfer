...
namespace EVATransfer.Framework
{
    internal static class ET_SkinsLibrary
    {
        #region Constructor
        static ET_SkinsLibrary()
        {
            _Initialized = false;
        ...
        {
            ET_MBE.LogFormatted_DebugOnly("Setting GUISkin to {0}", DefaultSkin);
        ...
        {
            ET_MBE.LogFormatted_DebugOnly("Setting GUISkin to {0}", SkinID);
            GUISkin OldSkin = _CurrentSkin;
            ...
            if (List.ContainsKey(SkinID))
                ...
                ET_MBE.LogFormatted("Unable to change GUISkin to {0}, GUISkin not found", SkinID);
        ...
        internal static GUISkin CopySkin(DefSkinType DefaultSkin)
        {
            switch (DefaultSkin)
            {
                case DefSkinType.Unity: return (GUISkin)ET_MBE.Instantiate(DefUnitySkin);
                case DefSkinType.KSP: return (GUISkin)ET_MBE.Instantiate(DefKSPSkin);
                //case DefSkinType.None: return new GUISkin();
                default: return (GUISkin)ET_MBE.Instantiate(DefKSPSkin);
            }
        }
        /// <summary>
        /// Copies a skin so you can make a new custom skin from an already defined one
        ...
        {
            if (List.ContainsKey(SkinID))
                return (GUISkin)ET_MBE.Instantiate(List[SkinID]);
            else
            {
                ET_MBE.LogFormatted("Unable to copy GUISkin to {0}, GUISkin not found", SkinID);
        ...
        internal static void AddStyle(ref GUISkin SkinToAction, GUIStyle NewStyle)
        {
            ...
            {
                ET_MBE.LogFormatted("No Name Provided in the Style to add to {0}. Cannot add this.", SkinToAction.name);
                return;
            }
